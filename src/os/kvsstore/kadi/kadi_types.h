...
struct oplog_page
{
	...
	oplog_page(int id_, unsigned char h, int buflen_, void *parent_):
			handle(h), buflen(buflen_), byteswritten(0),
			bufoffset(0), end(false), id (id_), parent(parent_), retcode(0), groupid(0), sequence(0)
	{
		buf = (buflen > 0)? (char*)malloc(buflen): 0;
	}
...
struct oplog_remove_ctx
{
	std::atomic<int> remained;
	std::atomic<int> inflight;
	oplog_remove_ctx(int num_deletes): remained(num_deletes), inflight(0) {}
...
struct oplog_info {
	uint8_t spaceid;
	uint32_t prefix;
	std::vector<oplog_page*> oplogpage_dir;
	std::vector<std::pair<void *, int>> oplogpage_keys;	// keys in the read buffers
	oploglist_t oplog_list;
	oplog_info(uint8_t spaceid_, uint32_t prefix_):
		spaceid(spaceid_), prefix(prefix_) {}
};
...
public:
	...
	bool get_ready_iterpages_events(std::vector<T *> &ctxs) {
		std::unique_lock<std::mutex> l(ready_lock);
