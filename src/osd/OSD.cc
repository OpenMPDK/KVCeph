...
OSD::OSD(CephContext *cct_, ObjectStore *store_,
         ...
         const std::string &dev, const std::string &jdev) : Dispatcher(cct_),
                                                            ...
                                                            authorize_handler_cluster_registry(new AuthAuthorizeHandlerRegistry(cct,
                                                                                                                                cct->_conf->auth_supported.empty() ? cct->_conf->auth_cluster_required : cct->_conf->auth_supported)),
                                                            authorize_handler_service_registry(new AuthAuthorizeHandlerRegistry(cct,
                                                                                                                                cct->_conf->auth_supported.empty() ? cct->_conf->auth_service_required : cct->_conf->auth_supported)),
                             ...
                             int currentactingprimary)
{
       ...
       e--)
  {
    ...
    // up set change?
    if ((up != currentup || upprimary != currentupprimary) && e > h.same_up_since)
    {
                                 ...
                                 const std::string &command, cmdmap_t &cmdmap, ostream &ss)
{
  ...
  if (command == "trigger_scrub")
  {
    ...
    if (pg->is_primary())
    {
      ...
      double scrub_max_interval = pool_scrub_max_interval > 0 ? pool_scrub_max_interval : g_conf->osd_scrub_max_interval;
...
void OSD::send_pg_stats(const utime_t &now)
{
  ...
  if (osd_stat_updated || !pg_stat_queue.empty())
  {
    ...
    while (!p.end())
    {
      ...
      ++p;
      if (!pg->is_primary())
      { // we hold map_lock; role is stable.
    ...
    do
    {
      ...
      if (pg->get_pgbackend()->scrub_supported() && pg->is_active())
      {
        dout(10) << "sched_scrub scrubbing " << scrub.pgid << " at " << scrub.sched_time
                 << (pg->scrubber.must_scrub ? ", explicitly requested" : (load_is_low ? ", load_is_low" : " deadline < now"))
                 << dendl;
        if (pg->sched_scrub())
        {
          pg->unlock();
                                     ...
                                     bool is_fast_dispatch)
{
      ...
      (map->get_cluster_addr(from) != m->get_source_inst().addr))
  {
    dout(5) << "from dead osd." << from << ", marking down, "
            ...
            << " expected " << (map->is_up(from) ? map->get_cluster_addr(from) : entity_addr_t())
            << dendl;
    ConnectionRef con = m->get_connection();
    con->mark_down();
    Session *s = static_cast<Session *>(con->get_priv());
...
void OSD::ShardedOpWQ::_process(uint32_t thread_index, heartbeat_handle_d *hb)
{
  ...
  assert(NULL != sdata);
  DERR << "ShardedOpWQ::_process - 1" << dendl;
  //osd->cct->kvsdbg.sendmsg_ex("shards = %d, thread_index = %d, shard_index = %d", num_shards, thread_index, shard_index);

  // peek at spg_t
  DERR << "ShardedOpWQ::_process - 2" << dendl;
  sdata->sdata_op_ordering_lock.Lock();
  DERR << "ShardedOpWQ::_process - 3" << dendl;
  if (sdata->pqueue->empty())
  {
    DERR << __func__ << " empty q, waiting" << dendl;
  ...
  DERR << "ShardedOpWQ::_process - 4" << dendl;
...
DERR << "ShardedOpWQ::_process - 7" << dendl;
  if (osd->is_stopping())
  {
    sdata->sdata_op_ordering_lock.Unlock();
  ...
  DERR << "ShardedOpWQ::_process - 8" << dendl;
  PGRef pg;
  uint64_t requeue_seq;
  {
    ...
    if (slot.waiting_for_pg)
    {
      ...
      DERR << __func__ << " " << item.first << " item " << item.second
  ...
  DERR << "ShardedOpWQ::_process - 9" << dendl;
  sdata->sdata_op_ordering_lock.Unlock();
  DERR << "ShardedOpWQ::_process - 10" << dendl;
  osd->service.maybe_inject_dispatch_delay();
  
  // [lookup +] lock pg (if we have it)
  if (!pg)
  {
    DERR << "ShardedOpWQ::_process - 11" << dendl;
    pg = osd->_lookup_lock_pg(item.first);
    DERR << "ShardedOpWQ::_process - 12" << dendl;
  }
  else
  {
    DERR << "ShardedOpWQ::_process - 13" << dendl;
    pg->lock();
    DERR << "ShardedOpWQ::_process - 14" << dendl;
  }

  osd->service.maybe_inject_dispatch_delay();

  //boost::optional<PGQueueable> qi;
  DERR << "ShardedOpWQ::_process - 15" << dendl;
  // we don't use a Mutex::Locker here because of the
  // osd->service.release_reserved_pushes() call below
  sdata->sdata_op_ordering_lock.Lock();
DERR << "ShardedOpWQ::_process - 16" << dendl;
  auto q = sdata->pg_slots.find(item.first);
  assert(q != sdata->pg_slots.end());
  auto &slot = q->second;
  --slot.num_running;
DERR << "ShardedOpWQ::_process - 17" << dendl;
  if (slot.to_process.empty())
  {
    // raced with wake_pg_waiters or prune_pg_waiters
    DERR << __func__ << " " << item.first << " nothing queued" << dendl;
    if (pg)
    {
      pg->unlock();
    }
    sdata->sdata_op_ordering_lock.Unlock();
    DERR << "ShardedOpWQ::_process - 18" << dendl;
    return;
  }
  if (requeue_seq != slot.requeue_seq)
  {
    DERR << __func__ << " " << item.first
    ...
    DERR << "ShardedOpWQ::_process - 19" << dendl;
    sdata->sdata_op_ordering_lock.Unlock();
    DERR << "ShardedOpWQ::_process - 20" << dendl;
    return;
  }
  DERR << "ShardedOpWQ::_process - 21" << dendl;
  ...
  dout(30) << __func__ << " " << item.first << " to_process " << slot.to_process
           << " waiting_for_pg=" << (int)slot.waiting_for_pg << dendl;
DERR << "ShardedOpWQ::_process - 22" << dendl;
...
DERR << "ShardedOpWQ::_process - 23" << dendl;
  // take next item
  ...
  if (!pg)
  {
    DERR << "ShardedOpWQ::_process - 24" << dendl;
    ...
    DERR << "ShardedOpWQ::_process - 25" << dendl;
    sdata->sdata_op_ordering_lock.Unlock();
    DERR << "ShardedOpWQ::_process - 26" << dendl;
    return;
  }
  sdata->sdata_op_ordering_lock.Unlock();
  DERR << "ShardedOpWQ::_process - 27" << dendl;
...
DERR << "ShardedOpWQ::_process - 28" << dendl;
  ...
  delete f;
  *_dout << dendl;
DERR << "ShardedOpWQ::_process - 29" << dendl;
  ThreadPool::TPHandle tp_handle(osd->cct, hb, timeout_interval,
                                 suicide_interval, thread_index);
  qi->run(osd, pg, tp_handle);
DERR << "ShardedOpWQ::_process - 30" << dendl;
  {
#ifdef WITH_LTTNG
    osd_reqid_t reqid;
    if (boost::optional<OpRequestRef> _op = qi->maybe_get_op())
...
DERR << "ShardedOpWQ::_process - 31" << dendl;
  pg->unlock();
DERR << "ShardedOpWQ::_process - 32" << dendl;
  threads_active.fetch_sub(1);
  ...
  if (_op)
  {
    ...
    if (op->rmw_flags != 0)
    {
      ...
      {
        DERR << "ShardedOpWQ::_process - 33 " << dendl;
        ceph_abort();
      }
    }
  }
  DERR << "ShardedOpWQ::_process - done" << dendl;
}
