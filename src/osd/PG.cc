...
{
  derr << __func__ << " PG::scrub started" << dendl;
  if (cct->_conf->osd_scrub_sleep > 0 &&
      (scrubber.state == PG::Scrubber::NEW_CHUNK ||
       scrubber.state == PG::Scrubber::INACTIVE) &&
       scrubber.needs_sleep) {
    ceph_assert(!scrubber.sleeping);
    dout(0) << __func__ << " state is INACTIVE|NEW_CHUNK, sleeping" << dendl;
  ...
  if (!is_primary() || !is_active() || !is_clean() || !is_scrubbing()) {
    derr << "scrub -- not primary or active or not clean" << dendl;
  ...
  chunky_scrub(handle);
  derr << __func__ << " PG::scrub finished" << dendl;;
}
...
void PG::chunky_scrub(ThreadPool::TPHandle &handle)
{
  ...
  if (scrubber.is_chunky_scrub_active())
  {
    ...
    {
      derr << "scrub  pg changed, aborting" << dendl;
      scrub_clear_state();
      scrub_unreserve_replicas();
      return;
    }
  ...
  while (!done)
  {
    ...
    case PG::Scrubber::NEW_CHUNK:
	   ...
	   */
        ...
        if (!_range_available_for_scrub(scrubber.start, candidate_end))
        {
          ...
          derr << __func__ << ": scrub blocked somewhere in range "
    ...
    case PG::Scrubber::WAIT_PUSHES:
      ...
      {
        derr<< "wait for pushes to apply" << dendl;
        done = true;
      }
      break;
    ...
    case PG::Scrubber::WAIT_LAST_UPDATE:
      ...
      else
      {
        ...
        derr << "wait for writes to flush" << dendl;
        done = true;
      }
      break;
    ...
    case PG::Scrubber::WAIT_REPLICAS:
      if (scrubber.waiting_on > 0)
      {
        ...
        derr << "wait for replicas to build scrub map" << dendl;
        done = true;
      }
      else
      {
    ...
    case PG::Scrubber::FINISH:
      ...
      if (!snap_trimq.empty())
      {
        derr << "scrub finished, requeuing snap_trimmer" << dendl;
    ...
    default:
      derr << __func__ << " calling abort " << dendl;
      ceph_abort();
    }
  }
